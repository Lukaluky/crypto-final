...
def sha256(message: bytes) -> str:
    ...
    # Round constants (simplified reuse)
    k = [
    ...
    for i in range(0, len(message), 64):
        ...
        for j in range(16, 64):
            s0 = (_right_rotate(w[j - 15], 7) ^
                  _right_rotate(w[j - 15], 18) ^
                  (w[j - 15] >> 3))
            s1 = (_right_rotate(w[j - 2], 17) ^
                  _right_rotate(w[j - 2], 19) ^
                  (w[j - 2] >> 10))
        ...
        for j in range(64):
            s1 = (_right_rotate(e, 6) ^
                  _right_rotate(e, 11) ^
                  _right_rotate(e, 25))
            ch = (e & f) ^ (~e & g)
            temp1 = (h0 + s1 + ch + k[j] + w[j]) & 0xffffffff
            s0 = (_right_rotate(a, 2) ^
                  _right_rotate(a, 13) ^
                  _right_rotate(a, 22))
            maj = (a & b) ^ (a & c) ^ (b & c)
